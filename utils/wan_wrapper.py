...
class WanTextEncoder(torch.nn.Module):
    def __init__(self) -> None:
        ...
        self.text_encoder.load_state_dict(
            torch.load("/mntc/code/self_forcing_orig/wan_models/Wan2.1-T2V-1.3B/models_t5_umt5-xxl-enc-bf16.pth",
                       map_location='cpu', weights_only=False)
        )

        self.tokenizer = HuggingfaceTokenizer(
            name="/mntc/code/self_forcing_orig/wan_models/Wan2.1-T2V-1.3B/google/umt5-xxl/", seq_len=512, clean='whitespace')
...
class WanVAEWrapper(torch.nn.Module):
    def __init__(self):
        ...
        self.model = _video_vae(
            pretrained_path="/mntc/code/self_forcing_orig/wan_models/Wan2.1-T2V-1.3B/Wan2.1_VAE.pth",
            z_dim=16,
        ).eval().requires_grad_(False)
    ...
    def __init__(
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                f"/mntc/code/self_forcing_orig/wan_models/{model_name}/", local_attn_size=local_attn_size, sink_size=sink_size)
        else:
            self.model = WanModel.from_pretrained(f"/mntc/code/self_forcing_orig/wan_models/{model_name}/")
        self.model.eval()
